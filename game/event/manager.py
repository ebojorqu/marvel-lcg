...
class EventManager:
    ...
    def ProcessPayingEffect(self, message: 'Message.WhenPlayerPayingResources', resources_effect: 'Effect', priority: 'TimingPriority') -> 'GAME_OVER':
        ...
        # Resources
        # Fix "53019": for Alliance cards, the resource can be generated by another
        # player's card. Re-check against that card's own controller (not the
        # player paying for the alliance card), so hero/alter-ego-form gated
        # resource abilities (e.g. "56029b Bruce Banner"'s "10008") evaluate the
        # correct player's form.
        control_by = resources_effect.this.GetControlByOrOwner()
        asked_player = control_by if isinstance(control_by, Player) else message.to_player
        resources_effects = EventManager.FilterAvailableEffects(message, [resources_effect], asked_player, self.world, None)

        if not resources_effects:
            return self.world.is_game_over

        selected_effect = None
        for effect in resources_effects:
            if effect == resources_effect:
                selected_effect = effect
                break
        if selected_effect is None:
            for effect in resources_effects:
                if effect.this == resources_effect.this:
                    selected_effect = effect
                    break
        if selected_effect is None:
            selected_effect = resources_effects[0]

        self.ProcessEffect(selected_effect, message, priority)
            ...
            def check_initiator() -> Tuple[bool, str]:
                ...
                if effect.ability.flags.is_resource or effect.ability.flags.is_discard_pay:
                    ...
                    if Event.IsType(message.for_effect.this) and message.for_effect.this.alliance:
                        # Fix "53019": the resource effect belongs to the card that is
